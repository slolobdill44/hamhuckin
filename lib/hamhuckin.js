function gameStart() {
  ...
  (function applyMobileScale() {
    ...
    function scale() {
      ...
      if (isLandscape) {
        // Fit within the space below the nav, centered vertically and horizontally
        var availableWidth = window.innerWidth - PADDING * 2;
        var availableHeight = window.innerHeight - navHeight - PADDING * 2;
        s = Math.min(availableWidth / 1050, availableHeight / 602);
        left = (window.innerWidth - 1050 * s) / 2;
        top = navHeight + PADDING + (availableHeight - 602 * s) / 2;
      } else {
        // Fit within viewport width, top-aligned just under the nav
        s = Math.min((window.innerWidth - PADDING * 2) / 1050, 1);
        left = (window.innerWidth - 1050 * s) / 2;
